...
namespace EIIP.DAO
{
	...
	public class CommonOracle : CommonInterface
	{
        ...
        public DataSet ExeForDst(String QueryString, OracleDataAdapter ad)
        {
            DataSet ds = new DataSet();
            //OracleDataAdapter ad = new OracleDataAdapter();
            ad.SelectCommand = cmd;
            cmd.CommandType = CommandType.Text;
            cmd.CommandText = QueryString;
            OracleCmdBuilder cb = new OracleCmdBuilder();
            cb.SetDataAdapter(ad);
        ...
        ///执行存储过程返回DataTable ExecuteSPSursor
        public DataSet ExecuteSPSursor(string StoredProcedureName, string[] Parameters, string[] ParametersValue, string[] ParametersType, string[] ParametersDirection)
        {
            try
            {
                ...
                for (int i = 0; i < Parameters.Length; i++)
                {
                    //OracleParameter myParm = this.cmd.Parameters.Add("@" + Parameters[i], Type.GetType(ParametersType[i].ToString()));
                    //myParm.Value = ParametersValue[i];
                    //myParm.OracleDbType = OracleDbType.RefCursor
                ...
                OracleDataAdapter _adapterObj = new OracleDataAdapter(this.cmd);
